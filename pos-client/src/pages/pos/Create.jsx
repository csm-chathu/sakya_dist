...
function CustomerCombobox({ customers, selected, onSelect }) {
  ...
  function choose(c) { onSelect(c); setQ(''); setOpen(false); }
    ...
    return (
      <div className="flex items-center gap-2 h-10 px-3 border border-slate-300 rounded-xl bg-orange-50">
        ...
        {selected.price_level && (
          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-orange-200 text-orange-700 uppercase">{selected.price_level}</span>
        )}
  ...
  return (
    <div ref={ref} className="relative">
      <input className={inputCls + ' w-full'} placeholder="Search customer…" value={q}
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg max-h-52 overflow-y-auto">
          <button type="button" onMouseDown={() => choose(null)}
          ...
          {filtered.length === 0
            ...
            : filtered.map(c => (
                <button key={c.id} type="button" onMouseDown={() => choose(c)}
                  ...
                  {c.price_level && (
                    <span className="ml-2 text-[10px] font-bold px-1 py-0.5 rounded bg-slate-100 text-slate-500 uppercase">{c.price_level}</span>
                  )}
                </button>
              ))
          }
...
function ProductCombobox({ products, selectedId, selectedName, onSelect }) {
  ...
  const filtered = q.trim()
    ? products.filter(p => {
        ...
        return p.name.toLowerCase().includes(lq) || (p.sku && p.sku.toLowerCase().includes(lq)) || (p.barcode && p.barcode.toLowerCase().includes(lq));
      })
    : products;

  function choose(p) { onSelect(p); setQ(''); setOpen(false); }
  function clear() { onSelect(null); setQ(''); }
  ...
  return (
    <>
      <input ref={inputRef} className={inputCls + ' w-full py-1.5'} placeholder="Search product…" value={q}
      ...
      {open && (
        <div ref={dropRef}
          ...
          {filtered.length === 0
            ? <div className="px-3 py-2 text-sm text-slate-400">No match</div>
            : filtered.map(p => (
                <button key={p.id} type="button" onMouseDown={() => choose(p)}
                  ...
                  <span className="text-xs text-slate-400 shrink-0">Rs. {Number(p.selling_price || 0).toLocaleString('en-LK')}</span>
                </button>
              ))
          }
        </div>
...
export default function PosCreate() {
  ...
  const [form, setForm] = useState({ delivery_date: '', notes: '', payment_method: 'cash', discount_pct: '', receipt_no: '' });
  ...
  function setItem(idx, field, value) {
    setItems(prev => prev.map((row, i) => {
      ...
      updated.total = (parseFloat(updated.qty) || 0) * (parseFloat(updated.selling_price) || 0);
      return updated;
    }));
  }
  ...
  function handleProductSelect(idx, p) {
    if (!p) {
      setItems(prev => prev.map((row, i) => i !== idx ? row : { ...row, product_id: '', product_name: '', selling_price: '', total: 0 }));
      return;
    }
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      items: validItems.map(r => {
        ...
        return { product_id: parseInt(r.product_id), product_name: r.product_name || p?.name || '', unit_price: price, cost_price: p?.cost_price || 0, qty, discount: 0, total: qty * price };
      }),
      payments: [{ method: form.payment_method, amount: total, reference: form.receipt_no || null }],
      subtotal, discount: discountAmt, tax: 0, extra_charges: 0, total, paid: total, balance: 0,
      status: 'completed', notes: form.notes || null, delivery_date: form.delivery_date || null,
    };

    try {
      const result = await createSale(payload).unwrap();
      navigate(`/pos/${result.id}`);
    } catch (e) {
      setError(e?.data?.error || 'Failed to create sale');
    }
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="mb-2">
        <h1 className="text-lg font-bold text-slate-800">New Sale</h1>
        <p className="text-sm text-slate-400">Create a new sales transaction.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" onKeyDown={e => { if (e.key === 'Enter' && e.target.type !== 'submit') e.preventDefault(); }}>
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <CustomerCombobox customers={customers} selected={selectedCustomer} onSelect={handleCustomerSelect} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">Delivery Date</label>
            <input type="date" className={inputCls + ' w-full'} value={form.delivery_date} onChange={e => setField('delivery_date', e.target.value)} />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-xs font-semibold text-slate-500 mb-1">Notes</label>
            <textarea className={inputCls + ' w-full resize-none'} rows={2} placeholder="Notes…" value={form.notes} onChange={e => setField('notes', e.target.value)} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <button type="button" onClick={addRow} className="px-3 py-1.5 text-xs font-semibold bg-orange-50 text-orange-600 border border-orange-200 rounded-lg hover:bg-orange-100 transition-colors">+ Add Item</button>
          </div>
                ...
                {items.map((row, idx) => (
                  <tr key={idx}>
                    <td className="px-4 py-2">
                      <ProductCombobox products={products} selectedId={row.product_id} selectedName={row.product_name} onSelect={p => handleProductSelect(idx, p)} />
                    </td>
                    <td className="px-4 py-2">
                      <input type="number" min="0.001" step="0.001" className={inputCls + ' w-full text-right'} value={row.qty}
                        onChange={e => setItem(idx, 'qty', e.target.value)} onFocus={e => e.target.select()}
                        ref={el => qtyRefs.current[idx] = el} required />
                    </td>
                    <td className="px-4 py-2">
                      <input type="number" min="0" step="0.01" className={inputCls + ' w-full text-right'} value={row.selling_price}
                        onChange={e => setItem(idx, 'selling_price', e.target.value)} onFocus={e => e.target.select()} required />
                    </td>
                    <td className="px-4 py-2 text-right font-semibold text-slate-800">
                      {Number(row.total || 0).toLocaleString('en-LK', { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-4 py-2 text-center">
                      <button type="button" onClick={() => removeRow(idx)} className="text-red-400 hover:text-red-600 transition-colors" title="Remove">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {PAYMENT_METHODS.map(m => (
                  <button key={m.value} type="button" onClick={() => setField('payment_method', m.value)}
                    className={`px-3 py-2 rounded-xl text-sm font-semibold border transition-colors ${form.payment_method === m.value ? 'bg-orange-500 text-white border-orange-500' : 'text-slate-600 border-slate-200 hover:bg-slate-50'}`}>
                    {m.label}
                  </button>
                ))}
              </div>
              {form.payment_method === 'bank_transfer' && (
                <div className="mt-2">
                  <input type="text" placeholder="Receipt / Reference No." className={inputCls + ' w-full'} value={form.receipt_no} onChange={e => setField('receipt_no', e.target.value)} />
                </div>
              )}
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1">Discount (%)</label>
              <input type="number" min="0" max="100" step="0.01" className={inputCls + ' w-full'} placeholder="0" value={form.discount_pct} onChange={e => setField('discount_pct', e.target.value)} />
            </div>
          </div>
          ...
          {error && <div className="px-4 py-2 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600">{error}</div>}

          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => navigate('/pos')} className="px-8 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-600 hover:bg-slate-50 transition-colors">Cancel</button>
            <button type="submit" disabled={submitting} className="px-10 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-sm font-bold transition-colors disabled:opacity-60">
