...
export default function PosIndex() {
  ...
  useEffect(() => {
    const onKey = e => {
      ...
      if (e.key === 'Enter') {
        ...
        if (bc.length >= 3) navigate('/pos/create', { state: { barcode: bc } });
        return;
      }
  ...
  const METHOD_STYLE = {
    ...
    qr: 'bg-purple-100 text-purple-700', credit: 'bg-red-100 text-red-600',
  };
  const payMethod = payments => {
    ...
    return (
      <span className="flex flex-wrap gap-1 justify-center">
        {methods.map(m => (
          <span key={m} className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${METHOD_STYLE[m] || 'bg-slate-100 text-slate-600'}`}>{m}</span>
        ))}
      </span>
    );
  };
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link to="/pos/create" className="px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors">
          + New Sale
        </Link>
      </div>
        ...
        {rows.map(s => (
          <Link key={s.id} to={`/pos/${s.id}`}
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-50">
              ...
              {canDelete && (
                <button onClick={e => { e.preventDefault(); e.stopPropagation(); handleDelete(s.id, s.invoice_no); }}
            ...
            <tbody>
              {rows.map(s => (
                <tr key={s.id} className="odd:bg-white even:bg-slate-50 hover:bg-blue-50 border-b border-slate-100 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center">{payMethod(s.payments)}</td>
                  <td className="px-4 py-3 text-center">{statusBadge(s.status)}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      <Link to={`/pos/${s.id}`} className="inline-flex items-center px-2.5 py-1 rounded-md border border-blue-200 bg-blue-50 text-xs font-medium text-blue-600 hover:bg-blue-100 transition-colors">{t('btn.view')}</Link>
                      <Link to={`/pos/${s.id}?print=1`} title="Reprint invoice" className="inline-flex items-center px-2 py-1 rounded-md border border-slate-200 bg-slate-50 text-xs font-medium text-slate-500 hover:bg-slate-100 transition-colors">
